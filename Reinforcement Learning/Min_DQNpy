...
def check_grads(model, inputs, targets, new_ep_values, rewards, grads):
    """
    ...
    for param, dparam, name in zip(params, gradients,
        ...
        for i in range(num_checks):
            ...
            test_act_target_qs = np.max(test_target_qs, axis=-1, keepdims=False)
...
for epoch in range(EPOCHS):
    ...
    targets, rewards, ep_values, new_ep_values = [], [], [], []
    ep_rewards = 0
    done = False
    ...
    act_target_qs = np.max(target_qs, axis=-1, keepdims=False)
    ...
    if check_gradients:
        ...
        difference = check_grads(model, memory['inputs'], targets, new_ep_values, rewards, grads)
        check_gradients = False
        time.sleep(5)
